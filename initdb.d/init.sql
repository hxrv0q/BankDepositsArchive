...
(
  ID            UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
  DepositorID   UNIQUEIDENTIFIER FOREIGN KEY REFERENCES Depositor (ID),
...
BEGIN TRY
    ...
    WHILE @counter <= 10
    BEGIN
        SET @depositorID = NEWID();
        INSERT INTO Depositor (ID, LastName, FirstName, Patronymic, PassportSeries, PassportNumber, HomeAddress)
        VALUES (@depositorID, CONCAT('LastName', @counter), CONCAT('FirstName', @counter), CONCAT('Patronymic', @counter), 'AB', CONCAT('1234', @counter), 'Some Address');
        ...
        DECLARE @randomDeposits INT = CAST(RAND() * 3 AS INT) + 1;

        WHILE @depositCounter <= @randomDeposits
        BEGIN
            INSERT INTO Deposit (ID, AccountID, Amount, Date)
            VALUES (NEWID(), @accountID, CAST(RAND() * 1000 AS MONEY), GETDATE());

            SET @depositCounter = @depositCounter + 1;
        END;
